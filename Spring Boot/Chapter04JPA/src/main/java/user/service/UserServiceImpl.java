...
public class UserServiceImpl implements UserService {
	...
	public void write(UserDTO userDTO) {
		...
		//똑같은 id가 없으면 insert 수행되고, id가 있으면 update 수행
	...
	public String isExistId(String id) {
		//DB
		Optional<UserDTO> userDTO = userDAO.findById(id); //select * from where id=?		
		System.out.println(userDTO); //id값이 없으면 Optional.empty 라고 출력된다.
		
		if(userDTO.isPresent()) // 값이 없으면 false - DB에 아이디가 없다 - 사용 가능 
			return "exist"; //id가 있다 - 사용 불가능
		else
			return "non_exist"; 
	}
	...
	public void delete(String id) {
		...
		//deleteById()는 내부적으로 findById() 수행하고 delete를 처리한다.
	...
	public List<UserDTO> getUserSearchList(String columnName, String value) {
		...
		*/
		...
		//@Query 어노테이션
		if(columnName.equals("name")) { 
			return userDAO.getUserSearhName(value); //사용자 메소드
			
		} else {
			return userDAO.getUserSearhId(value); //사용자 메소드
